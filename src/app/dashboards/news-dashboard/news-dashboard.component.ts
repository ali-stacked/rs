...
export class NewsDashboardComponent implements OnInit {
  ...
  constructor(
    ...
    this.current = route.snapshot.data['data'].covid;
    ...
    this.tasks = route.snapshot.data['data'].tasks;
     ...
     this.dec = route.snapshot.data['data'].dec;
     this.nov = route.snapshot.data['data'].nov;
     this.jan = route.snapshot.data['data'].jan;
...
// replace with covid properties and map names
    this.statistics = [
      {
        name: 'Total Deaths',// date
        value: this.current['death'],
        valueChange: this.current['deathIncrease'],
        positive: false,
        miniChartOptions: this.leadsMiniChartOptions
      },
      {
        name: 'Death Increase',// date
        value: this.current['deathIncrease'],
        valueChange: 54,
        positive: true,
        miniChartOptions: this.prospectsMiniChartOptions
      },
      {
        name: 'New Positive Tests', // date
        value: this.current['positiveIncrease'],
        valueChange: 19,
        positive: true,
        miniChartOptions: this.clientsMiniChartOptions
      },
      {
        name: 'Total Hospitalizations', // date
        value: this.current['hospitalized'],
        valueChange: '5%',
        positive: true,
        miniChartOptions: this.leadsToProspectsMiniChartOptions
      },
      {
        name: 'Increase In Hospitalizations', // date
        value: this.current['hospitalizedIncrease'],
        valueChange: '6%', // percentage change over previous value
  ...
  ngOnInit() {
    ...
    this.getTodaysDate();
    this.getCovid();
    // this.getCurrentDeaths(this.covid)
  ...
  getCovid(): void {
    this.newsService.getCovidData().subscribe(res =>{
      this.covid = res;
    })
  }
